...
const TimelineSeasonBasespotsList = ({
  ...
  useLayoutEffect(() => {
    supabase
      .rpc("get_basespot_thumbnails", {
        ids: timelineSeasonBasespotsByTimelineSeasonId
          .map((f) => f.id.toString())
          .join(","),
      })
      .then(({ count, data, error, status, statusText }) => {
        ...
        setImages(
          data.map((d) =>
            d.path?.includes(".emptyFolderPlaceholder")
              ? "https://xyhqysuxlcxuodtuwrlf.supabase.co/storage/v1/object/public/timelineimages/13/20220618173551_1.jpg"
              : d.path,
          ),
        );
      });
