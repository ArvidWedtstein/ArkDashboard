...
const ItemForm = (props: ItemFormProps) => {
  ...
  const onSubmit = (data: FormItem) => {
    ...
    data.recipe = recipe.map((item) => ({ itemId: item.id, count: item.amount }));
  ...
  let [recipe, setRecipe] = useReducer(reducer, props.item?.recipe ? (props.item?.recipe as any[]).map((f) => {
                ...
                <div className="flex flex-col mt-2">
                  {recipe.map((rec) => (
                    <div className="rw-button-group !mt-0 mb-0 justify-start text-sm font-medium">
                      <span onClick={() => { setRecipe({ type: "REMOVE", id: rec.id }) }} className="rw-input mt-0 inline-flex items-center min-w-full">
                        <img className="h-4 mr-2" src={rec.image} alt={rec.name} />
                        <span className="block truncate text-left">{rec.name}</span>
                      </span>
                      <input className="rw-input mt-0 w-20" defaultValue={rec.amount} onChange={(e) => { setRecipe({ type: 'UPDATE_AMOUNT', item: rec, amount: e.target.value }) }} />
                    </div>
                  ))}
                </div>
