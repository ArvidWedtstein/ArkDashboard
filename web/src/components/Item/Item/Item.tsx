  ...
  return (
    <>
      <div className="rw-segment">
        <section className="grid grid-cols-2 my-3 rounded-md bg-stone-200 p-4 text-gray-700 dark:bg-zinc-600 dark:text-white">
          <div className="">
            <img
              ...
              {item.recipe && item.crafting_time && (
                <>
                  <div className="mr-4 mb-4 inline-block">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-current inline-block" viewBox="0 0 384 512">
        ...
        <section className="my-3 grid grid-cols-2 gap-4 rounded-md text-gray-700 dark:text-white">
          {(item.DinoEffWeight && item.DinoEffWeight.filter((g) => g.is_gather_eff).length > 0) && (
            <div className="rounded-md bg-stone-200 p-4 dark:bg-zinc-600">
              <p className="my-1 text-lg">Gather Efficiency</p>
              <div className="flex flex-col">
                {item.DinoEffWeight.filter((g) => g.is_gather_eff).sort((a, b) => b.value - a.value).slice(0, 10).map((eff) => (
                  <div className="flex items-center">
                    <p className="mr-2 w-40 text-sm">{eff.name}</p>

                    <div className="flex h-2 w-32 flex-row divide-x divide-black rounded-full bg-gray-300">
                      {Array.from(Array(5)).map((_, i) => (
                        <div
                          className={clsx(
                            `w-1/5 h-full first:rounded-l-full last:rounded-r-full`,
                            {
                    ...
                    <p className="ml-2 w-40 text-sm" title={eff.value}>#{eff.rank}</p>
                  </div>
                ))}
              </div>
            </div>
