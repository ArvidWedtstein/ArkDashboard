...
const Admin = ({ basespots, profiles, roles }: FindAdminData) => {
  const [updateUser, { loading, error }] = useMutation(UPDATE_PROFILE_MUTATION, {
    onError: (error) => {
      ...
      <div className="container-xl m-4 overflow-hidden p-3 text-center">
        <div className="mb-3 flex flex-col-reverse space-x-3 md:flex-row">
          ...
          <Chart
            ...
            className="bg-gray-200 text-black shadow-lg transition ease-in-out dark:bg-zinc-700 dark:text-white rounded-lg border border-transparent"
            height={100}
            data={Object.values(groupDatesByMonth(profiles.map(p => new Date(p.created_at)).sort((a, b) => a.getTime() - b.getTime()))).map((g: Date[]) => g.length)}
            labels={Object.keys(groupDatesByMonth(profiles.map(p => new Date(p.created_at)).sort((a, b) => a.getTime() - b.getTime()))).map(p => new Date(p).toLocaleDateString('en-GB', { month: 'short' }))} 
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              render: ({ value, row }) => (
                <div className="inline-flex gap-x-2">
                  {/* Bans for one week */}
                  <button disabled={loading} className="rw-button rw-button-small rw-button-red-outline group" onClick={() => {
                    toast.custom((t) => (
                      <div className={`${t.visible ? 'animate-fly-in' : 'animate-fade-out'} p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-zinc-800 dark:text-red-400 dark:border-red-800`} role="alert">
                        <div className="flex items-center">
                          ...
                          <h3 className="text-lg font-medium">You're about to ban <b>{row.username}</b></h3>
                        </div>
                        <div className="mt-2 mb-4 text-sm">
                          <p>Are you sure you want to ban {row.username} for one week?</p>
                        </div>
                        <div className="flex">
                          <button type="button" onClick={() => {
                            toast.dismiss(t.id)
                            toast.promise(updateUser({ variables: { id: value.toString(), input: { banned_until: new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 7) } } }), {
                              loading: 'Banning user...',
                              success: ({ data }) => `Successfully banned ${data.updateProfile.username}`,
                              error: ({ data }) => `Failed to ban ${data.updateProfile.username}`
                            });
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="w-5 h-5 transition ease-in-out duration-75 group-hover:rotate-45">
                      <path d="M512 208.3c0-9.103-7.43-16-15.99-16c-4.091 0-8.183 1.562-11.31 4.688l-12.7 12.7L302.4 40l12.68-12.69C318.2 24.19 319.8 20.09 319.8 16c0-9.103-7.43-16-15.99-16c-4.091 0-8.183 1.562-11.31 4.688l-143.9 144C145.5 151.8 143.9 155.9 143.9 160c0 9.103 7.43 16 15.99 16c4.091 0 8.183-1.562 11.31-4.688l12.68-12.69l73.39 73.44l-75.46 78.11L172.4 300.7c-8.456-8.437-19.56-12.67-30.64-12.67c-11.13 0-22.26 4.234-30.73 12.67l-98.31 98.38C4.232 407.6 0 418.7 0 429.8s4.232 22.23 12.7 30.7l38.76 38.78C59.65 507.5 70.55 512 82.14 512c11.6 0 22.5-4.5 30.69-12.72l98.31-98.34c8.464-8.469 12.7-19.59 12.7-30.7S219.6 348 211.1 339.5l-6.711-6.719l75.47-78.12l73.56 73.6l-12.66 12.67c-3.123 3.125-4.685 7.219-4.685 11.31c0 9.103 7.43 16 15.99 16c4.092 0 8.183-1.562 11.31-4.688l143.9-144C510.4 216.5 512 212.4 512 208.3zM191.9 370.2c0 2.922-1.113 5.844-3.338 8.078l-98.32 98.34c-2.139 2.156-5.095 3.234-8.054 3.234c-2.959 0-5.923-1.078-8.078-3.234l-38.76-38.78c-2.225-2.234-3.338-5.156-3.338-8.078s1.113-5.844 3.338-8.078l98.31-98.38c2.218-2.219 5.138-3.312 8.074-3.312c2.92 0 5.856 1.094 8.089 3.344l38.74 38.78C190.7 364.4 191.9 367.3 191.9 370.2zM206.5 136l73.33-73.38l169.6 169.7l-73.33 73.38L206.5 136z" />
                    </svg>
                  </button>
                </div>
            ...
            profiles.some(p => p.banned_until != null) && {
              ...
              datatype: 'date',
              render: ({ value }) => (
                value && <span className="rw-badge rw-badge-gray-outline">
                  <svg
                    className="mr-1.5 h-2.5 w-2.5"
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
            ...
            },
            {
              ...
              field: 'role_id',
              sortable: true,
              render: ({ value, row }) => ( // TODO: find better toast colors / make rw-toast class
                <select className="rw-input rw-input-small" disabled={loading} defaultValue={value} onChange={(e) => {
                  toast.custom((t) => (
                    <div className={`${t.visible ? 'animate-fly-in' : 'animate-fade-out'} p-4 mb-4 text-pea-800 border border-pea-300 rounded-lg bg-pea-50 dark:bg-zinc-700 dark:text-pea-400 dark:border-pea-800`} role="alert">
                      <div className="flex items-center">
                        <svg className="flex-shrink-0 w-4 h-4 mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                        </svg>
                        <span className="sr-only">Info</span>
                        <h3 className="text-lg font-medium">You're about to change the role <b>{row.username}</b></h3>
                      </div>
                      <div className="mt-2 mb-4 text-sm">
                        <p>Are you sure you want to change {row.username}'s role from {roles.find(r => r.id == value)?.name} to {roles.find(r => r.id == e.target.value)?.name}?</p>
                      </div>
                      <div className="flex gap-x-2">
                        <button type="button" onClick={() => {
                          toast.dismiss(t.id)
                          toast.promise(updateUser({ variables: { id: row.id, input: { role_id: e.target.value } } }), {
                            loading: 'Changing role...',
                            success: ({ data }) => `Successfully Updated ${data.updateProfile.username}`,
                            error: ({ data }) => `Failed to update ${data.updateProfile.username}`
                          });
