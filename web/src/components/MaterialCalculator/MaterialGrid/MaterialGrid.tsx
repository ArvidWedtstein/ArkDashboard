...
const TreeBranch = memo(
  ({
    ...
    item: RecipeState;
    items: MaterialGridProps["craftingItems"];
  }) => {
    const {
      id,
      name,
    ...
    return (
      <li className="relative -ml-1 -mr-1 inline-block list-none px-2 pt-4 text-center align-top before:absolute before:top-0 before:right-1/2 before:h-4 before:w-1/2 before:border-t before:border-zinc-500 before:content-[''] after:absolute after:top-0 after:right-auto after:left-1/2 after:h-4 after:w-1/2 after:border-t after:border-l after:border-zinc-500 after:content-[''] first:before:border-0 first:after:rounded-tl-2xl last:before:rounded-tr-2xl last:before:border-r last:before:border-zinc-500 last:after:border-0 only:pt-0 only:before:hidden only:after:hidden">
        {item && (
          <Button
            className="flex aspect-square w-full max-w-[5rem] flex-col items-center justify-center text-xs"
            variant="outlined"
            color="DEFAULT"
            to={routes.item({ id })}
            title={
              craftingStation
                ? `${item.name} is crafted in ${craftingStation?.name}`
                : ""
            }
          >
            <img
              ...
              alt={name}
            />
            <span className="sr-only box-border w-full max-w-max whitespace-pre-wrap break-words text-xs">
              {name}
            </span>
            ...
            <Badge
              size="small"
              content={
                crafting_time === 0 ? 0 : timeFormatL(crafting_time, false)
              }
              variant="standard"
              color="none"
              className="![&>span]:items-end absolute inset-0 w-full lowercase [&>span]:mt-2 [&>span]:translate-x-0 [&>span]:place-content-end [&>span]:justify-end [&>*]:!text-right"
            />
            <Badge
              size="small"
              content={amount}
              ...
              max={100000000}
              className="absolute inset-0 w-full [&>span]:mb-2.5 [&>span]:mr-0.5 [&>span]:translate-x-0 [&>span]:place-content-end [&>span]:text-right"
            />
          </Button>
        )}
        {children?.length > 0 && (
...
export const MaterialGrid = ({ craftingItems, error }: MaterialGridProps) => {
      ...
      case "ADD": {
        ...
        return [
          ...state,
          {
            ...item,
            amount:
              amountToAdd % item?.itemRecipes[0]?.yields === 0
                ? amountToAdd
                : amountToAdd +
                  (payload.item?.itemRecipes[0]?.yields -
                    (amountToAdd % item?.itemRecipes[0]?.yields)),
          },
        ];
      }
      case "EDIT_AMOUNT": {
        ...
        return state.map((item, index) => {
          ...
          const recipeYields =
            item.itemRecipes[0]?.yields *
              crafting_station.item_production_multiplier || 1;

          const correctedAmount =
            amount < item.amount
              ? item.amount - recipeYields
              : item.amount + recipeYields;
  ...
  const calculateMaterials = (...objects: RecipeState[]): RecipeState[] => {
    ...
    const calculateItemRecipe = (
      ...
      const children = ItemRecipeItem.map((itemRecipeItem) => {
        ...
        const modifiedAmount =
          (itemRecipeItem.amount *
            amount *
            (resource_consumption_multiplier / item_production_multiplier)) /
          yields;

        if (
          resourceItemRecipe &&
          resourceItemRecipe.ItemRecipeItem.length > 0
        ) {
      ...
      return {
        ...
        children,
        crafting_time:
          (amount / item?.itemRecipes[0].yields) *
          (crafting_time / crafting_speed_modifier), // OLD: amount * (crafting_time / crafting_speed_modifier),
  ...
  const calculatedRecipes = useMemo(() => {
    const materials = calculateMaterials(...recipes);
  ...
  const handleCraftingStationChange = (_, value: string) => {
    setCraftingStations((prevCraftingStations) => {
      ...
      Object.keys(prevCraftingStations).forEach((category) => {
        if (
          prevCraftingStations[category].some((s) => s.id === Number(value))
        ) {
          prevCraftingStations[category] = prevCraftingStations[category].map(
            (s) => {
              return {
                ...s,
                active: !s.active,
    ...
    <div className="mx-1 flex w-full max-w-full flex-col gap-3">
      ...
      <section className="flex h-full w-full flex-col gap-3 sm:flex-row">
        <div className="flex flex-col space-y-1">
          ...
          <Card variant="outlined" className="max-w-sm p-2">
            <Input
              margin="none"
              size="small"
              className="mb-2"
            ...
            <TreeView
              options={Object.entries(
                ...
                .map(([category, categoryRecipes]) => {
                  return {
                    ...
                    children:
                      categoryRecipes.every(({ type }) => !type) ||
                      categoryRecipes.length < 5
                        ...
                        : Object.entries(groupBy(categoryRecipes, "type"))
                            .sort()
                            .map(([type, typeRecipes]) => ({
                              label: type,
              ...
              getOptionCaption={(opt, level) =>
                opt?.children && level > 0
                  ? formatNumber(opt?.children?.length)
                  : ""
              }
        ...
        <div className="w-full overflow-hidden">
          <div className="flex flex-row items-stretch space-x-3">
                ...
                >
                  {stations.map((station) => {
                    ...
                    return (
                      ...
                      >
                        <img
                          src={`https://xyhqysuxlcxuodtuwrlf.supabase.co/storage/v1/object/public/arkimages/Item/${
                            stationItem?.image || categoriesIcons["Other"]
                          }`}
                          loading="lazy"
                          className="w-12"
                        />
            ...
            <div>
              <ToggleButton
                className="h-full"
                selected={viewBaseMaterials}
                onChange={() => setViewBaseMaterials(!viewBaseMaterials)}
                value={"showBaseMaterials"}
              >
          ...
          <Table
            className="animate-fade-in mt-2 !divide-opacity-50 whitespace-nowrap"
