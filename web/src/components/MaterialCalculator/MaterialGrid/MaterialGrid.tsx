    ...
    <Form onSubmit={onAdd} error={error}>
      ...
      {item.length > 0 &&
        <>
          ...
          <LineChart className="my-4 animate-fade-in" items={mergeItemRecipe(viewBaseMaterials, ...item).map(({ amount, name, color }) => {
            return {
              name: name,
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                ...
                renderCell: ({ row }) => {
                  return (
                    mergeItemRecipe(viewBaseMaterials, { itemId: row.itemId, amount: row.amount }).sort((a, b) => a.itemId - b.itemId).map((itm, i) => (
