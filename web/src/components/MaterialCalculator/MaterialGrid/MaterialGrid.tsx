...
export const MaterialGrid = ({ error }: MaterialGridProps) => {
  ...
  const toggleBaseMaterials = useCallback((e) => {
      ...
      <div className="relative flex flex-row space-x-3">
        ...
        <div className="inline-flex rounded-md shadow-sm" role="group">
          <button data-testid="turrettowerbtn" type="button" onClick={addTurretTower} className="first:rounded-l-lg last:rounded-r-lg rounded-none inline-flex items-center rw-button rw-button-green"> {/*py-2 px-4 text-sm font-medium text-gray-900 bg-white border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-secondary dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white*/}
            <svg xmlns="http://www.w3.org/2000/svg" className="mr-2 w-4 h-4 fill-current stroke-current" fill="currentColor" viewBox="0 0 512 512">
          ...
          <button type="button" onClick={clear} className="first:rounded-l-lg last:rounded-r-lg rounded-none inline-flex items-center rw-button rw-button-red">
            <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="mr-2 w-4 h-4 fill-current" fill="currentColor" viewBox="0 0 352 512">
            ...
            caption={{
              ...
              content: (
                <div className="flex items-center">
                  <CheckboxField name="flexCheckDefault" className="rw-checkbox" onChange={toggleBaseMaterials} />
            ...
            columns={[
              {
                ...
                renderCell: ({ row, rowIndex }) => {
                  return (
                    <button type="button" onClick={() => onRemove(rowIndex)} className="hover:bg-red-500 relative rounded-full w-10 h-10 flex items-center justify-center">
                      <ImageField className="w-8 h-8" name="itemimage" src={"https://www.arkresourcecalculator.com/assets/images/80px-" + row.image} />
                    </button>
                  )
                },
              },
              {
                ...
                renderCell: ({ row, rowIndex }) => {
                  return (
                    <div className="flex flex-row items-center" key={`${row.itemId}+${Math.random()}`}>
                      <button type="button" className="border border-black dark:border-white relative dark:text-white text-black hover:bg-white hover:text-black mx-2 rounded-full w-8 h-8 text-lg font-semibold" onClick={() => onRemoveAmount(rowIndex)}>
                        -
                      </button>
                      <p
                      ...
                      >{row.amount}</p>
                      <button type="button" className="border border-black dark:border-white relative dark:text-white text-black hover:bg-white hover:text-black mx-2 rounded-full w-8 h-8 text-lg font-semibold" onClick={() => onAddAmount(rowIndex)}>
                        +
                      </button>
                    </div>
                  )
