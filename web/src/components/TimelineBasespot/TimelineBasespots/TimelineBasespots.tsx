...
const TimelineBasespotsList = ({
  ...
  const handleOnUp = useCallback(() => {
    ...
    imgTrack.current.dataset.prevPercentage = imgTrack.current.dataset.percentage;
  }, []);

  const handleOnMove = e => {
    if (imgTrack.current.dataset.mouseDownAt === "0") return;
    const mouseDelta = parseFloat(imgTrack.current.dataset.mouseDownAt) - parseFloat(e.clientX),
    ...
    <div className="rw-segment relative">
      <section className="rw-segment h-[40rem]">
        <div
          ...
          className="min-w-[100vw] h-full left-1/2 top-1/2 flex flex-row items-stretch absolute cursor-grab select-none space-x-3 p-3 will-change-scroll touch-pan-x overflow-x-auto"
          id="image-track"
        ...
        >
          {timelineBasespots.length > 0 && timelineBasespots.map((timelineBasespot, i) => (
            <div
              key={i}
              aria-controls={`tab-${i}`}
              className="flex flex-col min-w-[50vmin] flex-1 image object-cover transition-all duration-300 relative rounded after:content-[''] after:absolute after:left-0 after:top-0 after:block after:h-full after:w-full after:rounded after:bg-gradient-to-b after:from-transparent after:to-black"
              draggable="false"
              style={{
            ...
            >
              <div className="flex flex-col items-start justify-end h-full px-8 py-4 text-stone-200 z-10">
                <p className="text-xl font-bold uppercase">
                  {timelineBasespot.tribeName}
                </p>
                <p className="text-base font-light">{timelineBasespot.Map.name} {timelineBasespot.basespot_id ? `- ${timelineBasespot.basespot.name}` : ''}</p>
                <hr className="bg-gray-100 my-2 h-[1px] w-full rounded border-0 dark:bg-stone-200" />
                <div className="flex flex-row items-center justify-between space-x-6 overflow-y-hidden w-full">
                  ...
                  <button onClick={() => navigate(routes.timelineBasespot({ id: timelineBasespot.id.toString() }))} className="rw-button rw-button-gray-outline !mr-2">View</button>
                </div>
              </div>
            </div>
          ))}
