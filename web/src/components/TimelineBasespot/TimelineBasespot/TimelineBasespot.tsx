...
const TimelineBasespot = ({ timelineBasespot }: Props) => {
  ...
  const initRaid = (id: DeleteTimelineBasespotMutationVariables["id"]) => {
    if (
      ...
      raidTimelineBasespot({ variables: { id } });
    }
  }
  ...
  return (
    <article className="rw-segment">
      ...
      <div className="m-2 block rounded-md text-white">
        ...
        {(isAuthenticated && !timelineBasespot.endDate && !timelineBasespot.raided_by && !timelineBasespot.raidcomment) && (
          <section className="body-font mx-4 border-t border-gray-700 text-gray-700 dark:border-gray-200 dark:text-neutral-200">
            <button className="rw-button rw-button-red-outline rw-button-large" onClick={() => initRaid(timelineBasespot.id)}>Raid
