      ...
      <ul className="relative w-full border-l border-zinc-600 py-3  pr-2 dark:border-zinc-300 space-y-1 ml-1">
        {timelineSeasonEvents &&
          Object.entries(groupBy(timelineSeasonEvents, "created_at")).map(
            ([date, timeGroup], i) => (
              <Fragment key={`date-${i}`}>
                <li key={`date-${i}`} className="mt-4 ml-4">
                ...
                {timeGroup.map(
                  ({ id, title, content, tags, created_at, images }, idx) => (
                    <li className="relative">
                      <span className="absolute -left-2.5 z-30 flex h-5 w-5 items-center justify-center rounded-full bg-zinc-600 dark:bg-zinc-300">
                        {tags && tags.includes("raid") && (
                          <svg
                            ...
                            viewBox="0 0 512 512"
                            className="h-3 w-3 dark:text-black text-white"
                            fill="currentColor"
                          >
                        ...
                        {tags && tags.includes("dino") && (
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 640 512"
                            className="text-white h-3 w-3 dark:text-black"
                            fill="currentColor"
                          >
                      ...
                      <Card className="group ml-4" variant="outlined">
                        <CardHeader
                          title={title}
