...
import { groupBy, relativeDate } from "src/lib/formatters";
...
const TimelineSeasonEventsList = ({
  ...
  return (
    <div className="max-h-[36rem] flex-auto overflow-y-auto rounded px-2 text-zinc-700 dark:text-white">
      {timelineSeasonEvents && (
              ...
              return (
                <Fragment>
                  {timeGroup.map(({ id, title, content, tags, created_at, images }, idx) => (
                    <TimelineItem>
                      <TimelineOppositeContent className="!m-0 !flex-none !p-0" />
                      <TimelineSeparator>
                        <TimelineDot variant="outlined" color="secondary" className="dark:text-white text-black">
                      ...
                      <TimelineContent>
                        ...
                        <Card className="group" variant="outlined">
                          <CardHeader
                            title={title}
                            subheader={new Date(created_at).toLocaleTimeString("en-GB", {
                              hour: "2-digit",
                              minute: "2-digit",
                            })}
                            action={<span className="text-xs align-middle">{relativeDate(created_at, "narrow")}</span>}
                          />
                          <CardContent>
                          ...
                          <CardActions className="flex-col !items-start justify-start">
                            <div className="flex">
                              {tags &&
                                tags.split(",").map((tag, index) => <Badge key={`event-${id}-tag-${index}`} standalone content={`#${tag}`} color="secondary" variant="outlined" />)}
                            </div>
                            <div className="inline-flex mt-2 space-x-1 invisible group-hover:visible">
                              <Badge standalone color="secondary" content={"Edit"} onClick={() => setOpenModal(id, "editevent")} />
                              <Badge standalone color="error" content={"Delete"} onClick={() => onDeleteClick(id, title)} />
                            </div>
                          </CardActions>
                        </Card>
                      </TimelineContent>
