...
const Dropdown = ({ }) => {
  ...
  const { currentUser, getCurrentUser, isAuthenticated, logIn, logOut } = useAuth()
  useEffect(() => {
    getCurrentUser()
  }, [currentUser])
  const handleOpen = () => {
  ...
  return (
    <div ref={ref} className=" flex items-center relative text-left mr-6">
      <div className="inline-block">
        <button onClick={handleOpen} type="button" className="inline-flex w-full justify-center rounded-md border border-gray-300 text-white px-4 py-2 text-sm font-medium shadow-sm focus:outline-none focus:ring-offset-gray-100" id="menu-button" aria-expanded="true" aria-haspopup="true">
