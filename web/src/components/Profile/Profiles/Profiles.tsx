...
const ProfilesList = ({ profiles }: FindProfiles) => {
  ...
  const [anchorRef, setAnchorRef] = useState<{
    ...
    id: string;
    open: boolean;
  }>({ element: null, id: null, open: false });
  ...
  const handleClose = (event: React.MouseEvent<Document>) => {
    ...
    setAnchorRef({ element: null, open: false, id: null });
  };
      ...
      <div className="mb-3 flex flex-wrap gap-3">
        {profiles
          ...
          .map((profile) => (
            <figure
              key={profile.id}
              className="font-montserrat relative w-full max-w-xs overflow-hidden rounded-lg bg-zinc-700 text-left leading-6 text-white"
            >
              <img
              ...
              <figcaption className="before:border-skew relative flex w-full flex-col bg-zinc-700 p-6 before:absolute before:left-0 before:bottom-full before:h-0 before:w-0 before:border-transparent before:border-l-zinc-700 before:content-['']">
                <img
                  ...
                  className="absolute left-6 bottom-full aspect-square h-fit max-h-[90px] w-full max-w-[90px] rounded-full opacity-100 shadow-lg"
                />
                    ...
                    <figure className="font-montserrat relative w-full max-w-xs rounded-lg bg-zinc-700 text-left leading-6 text-white">
                      ...
                      <figcaption className="before:border-skew relative w-full bg-zinc-700 before:absolute before:right-0 before:left-0 before:bottom-full before:h-0 before:w-0 before:border-transparent before:border-l-zinc-700 before:content-['']">
                        <img
                          ...
                          alt={row.full_name}
                          className="absolute left-6 bottom-full aspect-square max-h-[90px] max-w-[90px] rounded-full opacity-100 shadow-lg"
                        />
      ...
      <Popper anchorEl={anchorRef?.element} open={anchorRef.open}>
        <ClickAwayListener onClickAway={handleClose}>
          <div
            ...
            onClick={() =>
              setAnchorRef({ element: null, id: null, open: false })
            }
          >
            <ul className="relative m-0 list-none py-2">
              <li>
                <Link
                  to={routes.profile({ id: anchorRef?.id })}
              ...
              {/* <li>
                <Link
                  to={routes.editProfile({ id: anchorRef?.id })}
                  className="relative box-border flex cursor-pointer select-none items-center justify-start whitespace-nowrap px-4 py-1.5 text-base font-normal text-current hover:bg-black/10 dark:hover:bg-white/10"
                >
                  <div className="inline-flex min-w-[36px] shrink-0">
                    <svg
              ...
              <li>
                <button
                  type="button"
                  onClick={() => onDeleteClick(anchorRef?.id)}
                  className="relative box-border flex cursor-pointer select-none items-center justify-start whitespace-nowrap px-4 py-1.5 text-base font-normal text-current hover:bg-black/10 dark:hover:bg-white/10"
                >
                  <div className="inline-flex min-w-[36px] shrink-0">
                    <svg
