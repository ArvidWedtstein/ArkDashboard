...
const Dino = ({ dino, itemsByIds }: Props) => {
    ...
    (state: DinoState, action: DinoAction) => {
      switch (action.type) {
        ...
        case "SELECT_FOOD":
          return {
            ...state,
            selected_food: [...state.selected_food, action.payload],
          };
        case "RECIPE_TAB_CHANGE":
          return {
            ...state,
            activeRecipeTabIndex: action.payload,
          };
        case "CHANGE_STAT":
          return {
            ...state,
            base_stats: state.base_stats.map((stat) => {
              if (stat.stat === action.payload.id) {
                stat.points = clamp(
  ...
  const genRandomStats = () => {
    ...
    let i =
      state.level -
      state.base_stats.map((b) => b.points).reduce((a, b) => a + b, 0);

    while (i > 0) {
      const stat = state.base_stats.filter((d) => d.stat != "t")[
        Math.floor(
          Math.random() * state.base_stats.filter((d) => d.stat != "t").length
        )
      ];
                              ...
                              <nav className="flex flex-row items-center space-x-2">
                                <button
                                  ...
                                  onClick={() =>
                                    dispatch({
                                      type: "CHANGE_STAT",
                                      payload: { id: row.stat, type: "remove" },
                                    })
                                  }
                                >
                                  -
                          ...
                          <ol className="w-full items-center justify-center space-y-4 sm:flex sm:space-x-8 sm:space-y-0">
                            {[
                              ...
                              },
                              {
                                name: "With Bonus",
                                sub: `Lvl ${
                                  state.level + tameData.levelsGained
                                }`,
                                icon: (
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
