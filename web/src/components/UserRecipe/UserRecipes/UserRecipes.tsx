...
`;
interface Props {
  userRecipes: FindUserRecipes["userRecipes"];
  onSelect?: (userRecipe: ArrayElement<FindUserRecipes["userRecipes"]>) => void;
}
const UserRecipesList = ({ userRecipes, onSelect }: Props) => {
  ...
  return (
    <div className="flex flex-row gap-3 overflow-x-auto dark:text-stone-100">
      {userRecipes.map(
        ({
          id,
          created_at,
          ...
          <div
            className="hover:border-pea-500 relative flex w-fit min-w-fit flex-row space-x-3 rounded-lg border border-zinc-500 border-transparent bg-zinc-300 p-4 shadow transition dark:bg-zinc-700"
            key={id}
          >
            <div
              onClick={() => {
...
z"
            ...
            {IsPrivate && (
              <div className="relative flex h-full flex-col items-center justify-center space-y-3 border-l border-zinc-500 pl-3">
                <button
                  onClick={() => navigate(routes.userRecipe({ id }))}
                  title="View"
                  className="rounded p-1.5 hover:bg-zinc-500 hover:bg-opacity-30 dark:hover:bg-zinc-300 dark:hover:bg-opacity-30"
                >
                ...
                {currentUser?.id == user_id && (
                  <>
                    <button
                      onClick={() => navigate(routes.editUserRecipe({ id }))}
                      title="Edit"
                      className="rounded p-1.5 hover:bg-zinc-500 hover:bg-opacity-30 dark:hover:bg-zinc-300 dark:hover:bg-opacity-30"
                    >
                    ...
                    <button
                      ...
                      title="Delete"
                      className="rounded p-1.5 hover:bg-zinc-500 hover:bg-opacity-30 hover:text-red-500 dark:hover:bg-zinc-300 dark:hover:bg-opacity-30"
                    >
