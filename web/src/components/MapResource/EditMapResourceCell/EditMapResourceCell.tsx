...
export const QUERY = gql`
  query EditMapResourceById($id: BigInt!) {
    mapResource: mapResource(id: $id) {
      id
      created_at
      updated_at
      map_id
...
export const Success = ({
  mapResource,
}: CellSuccessProps<EditMapResourceById>) => {
  const [updateMapResource, { loading, error }] = useMutation(
  ...
  const onSave = (
    ...
    toast.promise(updateMapResource({ variables: { id, input } }), {
      loading: 'Updating MapResource...',
      success: 'MapResource successfully updated',
      error: <b>Failed to update MapResource.</b>,
    })
  }
    ...
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">
          Edit MapResource {mapResource?.id}
        </h2>
      </header>
