...
const TimelineSeasonPeopleList = ({
  ...
  const [deleteTimelineSeasonPerson] = useMutation(
    DELETE_TIMELINE_SEASON_PERSON_MUTATION,
    {
      ...
      refetchQueries: [{ query: QUERY, variables: { timeline_season_id: timelineSeasonPeople[0].timeline_season_id } }],
  ...
  const onDeleteClick = (
    ...
    if (
      confirm(
        'Are you sure you want to delete timelineSeasonPerson ' + ingame_name + '?'
      )
    ) {
  ...
  return (
    <div className="flex justify-start gap-3 overflow-x-auto px-6">
      {timelineSeasonPeople.map(({ id, user_id, ingame_name, Profile }) => (
        <button className="flex-none py-6 px-3 group relative" key={id} onClick={() => {
          ...
          <div className="flex flex-col items-center justify-center gap-3 ">
            ...
            <img
              className="relative h-16 w-16 rounded-full group-hover:ring group-hover:ring-red-500 transition ease-in-out duration-150"
