...
import Collapse from "../Util/Collapse/Collapse";
import List, { ListItem } from "../Util/List/List";
...
const Sidebar = memo(({ }) => {
  ...
  const ActiveLink = ({ name, icon, to, activeClassName }: { name: string, icon: string, to: string, activeClassName: string }) => {
    ...
    return (
      ...
      >
        <Text variant="body1" className={clsx("capitalize transition-opacity ease-in-out duration-200", {
          "opacity-0": !open,
          "opacity-100": open
        })}>{name}</Text>
      </ListItem>
    ...
    <Fragment>
      {/* <aside className="group sticky top-0 bottom-0 z-10 overflow-x-auto border-gray-700 bg-zinc-800 py-2 dark:border-zinc-300 max-sm:border-b sm:h-auto sm:max-w-sm sm:overflow-visible sm:border-r sm:py-2 sm:px-4 md:min-w-[12rem]">
        <nav className="sticky top-0 bottom-0 flex w-full flex-row items-start justify-between space-y-1.5 sm:flex-col sm:justify-start">
          <div className="flex items-center justify-center border-zinc-300 text-black text-[#ffffffcc] transition-all sm:my-3 sm:w-full sm:flex-col">
            <Avatar
              url={
            ...
            >
              ...
              <hr role="seperator" className="rw-divider w-full bg-secondary-500 h-px" />
          ...
          <NavLink
            ...
            matchSubPaths={true}
            className="relative flex w-full flex-auto items-center justify-start space-x-3.5 rounded py-2 px-2.5 text-left text-white outline-none hover:bg-zinc-400/30 hover:text-gray-100 dark:hover:bg-zinc-400/30 dark:hover:text-white"
          >
            {Icon("Home")}
            {open && <span className="hidden md:block">Home</span>}
          </NavLink>
          <NavLink
            to={routes.basespots()}
            title={"Basespots"}
          ...
          >
            {Icon("Basespot")}
            {open && <span className="hidden md:block">Basespots</span>}
          </NavLink>
          <NavLink
          ...
          >
            {Icon("Crafting")}
            {open && <span className="hidden md:block">Crafting</span>}
          </NavLink>
          <NavLink
          ...
          >
            {Icon("Story")}
            {open && <span className="hidden md:block">Seasons Timeline</span>}
          </NavLink>
          <NavLink
            to={routes.dinos()}
            title={"Dinos"}
          ...
          >
            {Icon("Dinos")}
            {open && <span className="hidden md:block">Dinos</span>}
          </NavLink>
          <NavLink
            to={routes.items()}
            title={"Items"}
          ...
          >
            {Icon("Items")}
            {open && <span className="hidden md:block">Items</span>}
          </NavLink>
          <NavLink
            to={routes.maps()}
            title={"Maps"}
          ...
          >
            {Icon("Maps")}
            {open && <span className="hidden md:block">Maps</span>}
          </NavLink>
          <NavLink
          ...
          >
            ...
            {open && <span className="hidden md:block">Lootcrates</span>}
          </NavLink>
          <NavLink
            to={routes.tribes()}
            title={"Tribes"}
          ...
          >
            {Icon("Tribes")}
            {open && <span className="hidden md:block">Tribes</span>}
          </NavLink>
        </nav>
      </aside> */}
