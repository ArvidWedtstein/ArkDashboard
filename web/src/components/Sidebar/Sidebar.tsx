  ...
  const icons = {
    ...
    lootcrates: (
      <>
        ...
        {/* <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" className="h-4 w-4 fill-current stroke-current ml-1">
...
const Sidebar = memo(({}) => {
  ...
  return (
    <aside className="group z-10 min-w-[12rem] overflow-x-auto border-gray-700 bg-zinc-800 py-2 dark:border-zinc-300 max-sm:border-b sm:h-auto sm:max-w-sm sm:overflow-visible sm:border-r sm:py-2 sm:px-4">
      <div className="sticky top-0 flex w-full flex-row items-start justify-between space-y-1.5 sm:flex-col sm:justify-start">
        ...
        {navigation.map((item, index) => (
          ...
          // <a
          //   className="flex w-full items-center gap-x-3.5 rounded-md py-2 px-2.5 text-sm text-slate-700 hover:bg-gray-100/30 dark:text-white"
          //   key={`sidebar-item-${index}`}
          // >
          //   {Icon(item.name)}
          //   {item.name}
