import type { FindItemRecipeItems } from 'types/graphql'

import { Link, routes } from '@redwoodjs/router'
import type { CellSuccessProps, CellFailureProps } from '@redwoodjs/web'

import ItemRecipeItems from 'src/components/ItemRecipeItem/ItemRecipeItems'

export const QUERY = gql`
  query FindItemRecipeItems {
    itemRecipeItems {
      id
      created_at
      updated_at
...
export const Empty = () => {
  return (
    <div className="text-center text-black dark:text-white">
      {'No itemRecipeItems yet. '}
      <Link to={routes.newItemRecipeItem()} className="rw-link">
        {'Create one?'}
      </Link>
    </div>
  )
...
export const Success = ({
  itemRecipeItems,
}: CellSuccessProps<FindItemRecipeItems>) => {
  return <ItemRecipeItems itemRecipeItems={itemRecipeItems} />
}
