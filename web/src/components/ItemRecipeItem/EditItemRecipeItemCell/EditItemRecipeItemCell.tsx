...
export const QUERY = gql`
  query EditItemRecipeItemById($id: String!) {
    itemRecipeItem: itemRecipeItem(id: $id) {
      id
      created_at
      updated_at
      item_recipe_id
...
export const Success = ({
  itemRecipeItem,
}: CellSuccessProps<EditItemRecipeItemById>) => {
  const [updateItemRecipeItem, { loading, error }] = useMutation(
  ...
  const onSave = (
    input: UpdateItemRecipeItemInput,
    id: EditItemRecipeItemById['itemRecipeItem']['id']
  ) => {
    toast.promise(updateItemRecipeItem({ variables: { id, input } }), {
      loading: 'Updating ItemRecipeItem...',
      success: 'ItemRecipeItem successfully updated',
      error: <b>Failed to update ItemRecipeItem.</b>,
    })
  }
    ...
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">
          Edit ItemRecipeItem {itemRecipeItem?.id}
        </h2>
      </header>
