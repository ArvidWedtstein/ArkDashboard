...
const DatePicker = ({
  ...
  const handleInputChange = (e) => {
    ...
    if (
      ...
      setInternalValue(
        parsedDate.toLocaleDateString(navigator && navigator.language, {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        }),
  ...
  useEffect(() => {
    setInternalValue(
      defaultValue.toLocaleDateString(navigator && navigator.language, {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
      }),
  ...
  return (
    ...
    >
      ...
      >
          ...
          renderSuffix={(state) =>
            variant === "outlined" ? (
              <fieldset
                aria-hidden
                className={clsx(
                  `border transition-colors ease-in duration-75 absolute text-left ${borders[disabled || state.disabled ? "disabled" : state.focused ? color : "DEFAULT"]} bottom-0 left-0 right-0 -top-[5px] m-0 px-2 rounded-[inherit] min-w-0 overflow-hidden pointer-events-none`,
                )}
              >
                <legend
                  className={clsx(
                    "w-auto overflow-hidden block invisible text-xs p-0 h-[11px] whitespace-nowrap transition-all",
                    {
                      "max-w-full": open || state.focused || state.filled,
                      "max-w-[0.01px]":
                        !state.focused && !state.filled && !open,
                    },
                  )}
                >
                  {label && label !== "" && (
        ...
        {helperText && (
          <p
            className="mt-0.5 text-left text-xs leading-6 tracking-wide text-black/60 dark:text-white/70"
            {...HelperTextProps}
          >
            {helperText}
          </p>
