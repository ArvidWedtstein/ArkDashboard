...
const Ripple = forwardRef(
  ({ center: centerProp = false }: { center?: boolean }, ref) => {
    ...
    const rippleRoot = `overflow-hidden pointer-events-none absolute z-10 inset-0 rounded-[inherit]`;
    const classes = {
      ripple: `absolute`,
      rippleVisible: `opacity-30 animate-[rippleenter_${DURATION}ms]`,
      ripplePulsate: `animate-[rippleenter_${DURATION / 2}ms]`,
      child: `opacity-100 block w-full h-full rounded-[50%] bg-white`,
      childLeaving: `animate-[rippleexit_${DURATION}ms_cubic-bezier(0.2,_0,_0.1,_0)_0ms_forwards]`, // opacity-0
      childPulsate: `absolute left-0 top-0 animate-[ripplepulsate_2500ms_ease_in_out_200ms_infinite]`,
    };
    ...
    //   element.addEventListener("mousedown", useRippleHandler("start"));
    //   element.addEventListener("mouseup", useRippleHandler("stop"));
    //   element.addEventListener("mouseout", useRippleHandler("stop"));
    //   element.addEventListener("mouseleave", useRippleHandler("stop"));

    //   return () => {
    //     element.removeEventListener("mousedown", (e) =>
    //       useRippleHandler("start", e)
    //     );
    //     element.removeEventListener("mouseup", (e) =>
    //       useRippleHandler("stop", e)
    //     );
