...
const Alert = ({ severity = "error", variant = "outlined", children }: AlertProps) => {
  ...
  const severityClasses = {
    ...
    info: {
      ...
      filled: '',
      outlined: 'border border-sky-400 text-sky-200 bg-transparent'
    },
    success: {
  ...
  const icon = {
    error: (
      <svg className="w-4 h-4 fill-current inline-block select-none shrink-0 transition-colors duration-200" focusable="false" aria-hidden="true" viewBox="0 0 24 24">
  ...
  return (
    <div className={clsx("rounded shadow-none py-1.5 px-4 flex text-sm", {
      [severityClasses[severity][variant]]: severity,
    })}>
      <div className={clsx("mr-3 py-2 flex text-base opacity-90", {
        [iconClasses[severity][variant]]: severity,
      })}>
        {icon[severity]}
      </div>
      <div className="py-2 min-w-0 overflow-auto">
        <div className="text-base -mt-0.5 font-medium">Success</div>
        {children && children}
      </div>
    </div>
  )
