...
const TreeView = <T extends { children?: any[]; checked?: boolean }>(
  ...
  const renderOption = (option: T, key: string, level: number) => {
    ...
    const caption =
      getOptionCaption?.(option as WithoutChildren<T>, level) || null;
    const label = getOptionLabel?.(option as WithoutChildren<T>, level) || null;
    const isExpanded = openItems.includes(key);
    return (
      <div
        className={clsx(
          "flex cursor-pointer items-center box-border px-2 w-full hover:bg-zinc-500/30 rounded",
          {
            "!pl-4": level > 0,
          },
        )}
      ...
      >
        <div className="mr-1 flex-shrink-0 justify-center w-4 flex">
          {option?.children ? (
            <svg
              className="w-5 h-5 inline-block fill-current flex-shrink-0 transition-colors duration-200 select-none"
              focusable="false"
              aria-hidden="true"
              viewBox="0 0 24 24"
            >
        ...
        <div className="pl-1 box-border min-w-0 w-full relative text-base">
          <div className="flex items-center p-1 pr-0">
            {icon && typeof icon === "string" ? (
              <img
                loading="lazy"
                className="w-5 h-5 fill-current mr-2 flex-shrink-0 inline-block select-none"
                src={icon}
              />
            ) : (
              icon
            ...
            <span className="m-0 text-inherit text-xs font-normal">
              {caption}
            </span>
            {option?.checked && (
              <div className="mr-2">
  ...
  return (
    <ul className={clsx("flex-grow-1 overflow-y-auto list-none", className)}>
      {!options.length && (
        <p className="text-center w-full text-secondary-300">
          No items selected
        </p>
      )}
      {options.map((option, optionIndex) => {
